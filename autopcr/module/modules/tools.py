    ...
    async def do_task(self, client: pcrclient):
        ...
        for unit_id in db.redeem_unit:
            ...
            unsatisfied = [db.memory_to_unit[item[i][1]] for i in id if 
                           res[i] > 0 和 
                           (db.memory_to_unit[item[i][1]] not in client.data。unit 或
            ...
            if do:
                for slot_info in info.slot_info:
                    if slot_info.slot_id == 1:
                        ...
                        while total_mana > 0:
                            mana = min(total_mana, client.data。设置。max_once_consume_gold。redeem_unit)
...
class ex_equip_info(Module):
    async def do_task(self, client: pcrclient):
        ...
        cnt = sorted( 
                ...
                if not cb_only 或 db.ex_equipment_data[ex.ex_equipment_id]。clan_battle_equip_flag)。items())，
...
class gacha_start(Module):
    ...
    async def do_task(self, client: pcrclient):
        ...
        real_exchange_id = 0
        if gacha_id == 120001:
            if not client.data。return_fes_info_list 或 all(item.end_time <= client.time for item in client.data。return_fes_info_list):
        ...
        try:
            while True:
                if single_ticket:
                    ...
                    if isinstance(resp, GachaIndexResponse) 和 resp.campaign_info 和 resp.campaign_info。fg10_exec_cnt:
                ...
                cnt += 1
                if not always 或 self.can_stop(reward.new_unit, db.gacha_exchange_chara[target_gacha.exchange_id if not real_exchange_id else real_exchange_id]):
                    break
        except:
            raise 
        finally:
...
class Arena(Module):
    ...
    async def do_task(self, client: pcrclient):
        ...
        if best_team_id >= 0 和 best_team_id < len(attack):
...
class pjjc_back(Arena):
    ...
    async def get_defend_from_info(self, info: GrandArenaSearchOpponent) -> List[List[int]]:
        ...
        if info.grand_arena_deck:
            if info.grand_arena_deck。first 和 info.grand_arena_deck。first[0]。id != 2:
                ret.append([unit.id for unit in info.grand_arena_deck。first])
            if info.grand_arena_deck。second 和 info.grand_arena_deck。second[0]。id != 2:
                ret.append([unit.id for unit in info.grand_arena_deck。second])
            if info.grand_arena_deck。third 和 info.grand_arena_deck。third[0]。id != 2:
...
class ArenaInfo(Module):
    ...
    async def get_user_info(self, client: pcrclient, viewer_id: int) -> str: 
        ...
        if user_name is None 或 not self.use_cache:
...
class get_need_memory(Module):
    async def do_task(self, client: pcrclient):
        ...
        msg = {}
        if consider == "地图可刷取":
            demand = [i for i in demand if i[0] in db.memory_hard_quest 或 i[0] in db.memory_shiori_quest]
        elif consider == "大师币商店":
    ...
    async def do_task(self, client: pcrclient):
        ...
        for i in range(0， len(party)， 6):
            ...
            for unit in own_unit:
                ...
                if can_change_star 和 star != now_star:
                    if star >= 3 和 star <= 5 和 现在_star >= 3 和 现在_star <= 5:
